...
import PuppyCard, { type PuppyCardData } from "@/components/PuppyCard";
import TrustStrip from "@/components/TrustStrip";
import { SITE_URL } from "@/lib/site";
import { useCurrentLitter, formatLongDate } from "@/hooks/useParents";

type Puppy = PuppyCardData & {
  dob: string | null;
...
function LitterPage() {
  ...
  const litterLabel = litter?.name ?? "Spring 2026 litter";
  ...
  useEffect(() => {
    ...
    async function fetchPuppies() {
      const { data, error } = await supabase
        .from(T.puppies)
        .select(
          "id,name,slug,sex,dob,ready_date,status,tier,price,temperament_tags,personality_bio,ideal_home,image_url,stripe_payment_link,priority_order,collar_color",
        )
        .neq("status", "parent")
        .order("priority_order", { ascending: true });

      if (!error && data) setPuppies(data as Puppy[]);
      setLoading(false);
    }
    ...
    <div className="min-h-screen">
          ...
          <div className="mt-8 inline-flex items-center gap-2 rounded-full bg-primary-foreground/10 px-6 py-3 text-primary-foreground">
            ...
            <span className="text-sm font-semibold">
              {loading
                ...
                : `${availableCount} available · ${litterLabel}`}
            </span>
          </div>
      ...
      <section className="py-20 lg:py-28">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-accent">The Litter</p>
            <h2 className="mt-3 font-display text-3xl font-bold text-foreground lg:text-4xl">
              Meet Our Puppies
            </h2>
            <p className="mx-auto mt-4 max-w-2xl text-muted-foreground">
              {loading
                ? "Loading the litter…"
                : `${availableCount} of ${puppies.length} puppies available · reserved puppies stay visible so you can see what's taken`}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <p className="mt-16 text-center text-muted-foreground">
              Puppies for this litter will be listed here soon.{" "}
        ...
        <div className="mx-auto max-w-3xl px-6 text-center">
          ...
          <div className="mt-10">
            <Link
              ...
              className="rounded-xl bg-accent px-8 py-4 text-sm font-bold uppercase tracking-wider text-accent-foreground shadow-wolf transition-all hover:brightness-110"
            >
              Apply for This Litter — Takes 3 Minutes
            </Link>
          </div>
        </div>
      </section>
